...
import { buildDreamRequirementsPreview } from "./_dream_requirements";
...
function PreviewStageMarker({
  ...
  return (
    <div className="relative flex w-4 shrink-0 items-center justify-center self-stretch">
      ...
      <div
        className={`relative z-10 flex h-3.5 w-3.5 shrink-0 items-center justify-center rounded-full border ${ring}`}
      >
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        <p
          className={`text-[9px] font-bold uppercase tracking-[0.14em] ${labelClass}`}
        >
          Этап {stage.order}
        </p>
        {variant === "start" && (
...
export function DreamPathPreview({
  preview,
  dreamStages,
}: DreamPathPreviewProps) {
  if (!preview) {
    return (
    ...
    >
      ...
      <div className="rounded-2xl border border-cyan-500/15 bg-gradient-to-br from-slate-800/90 via-slate-800/70 to-slate-900/80 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <h4 className="mt-0.5 text-lg font-bold leading-tight text-white">
              {preview.title}
            </h4>
      ...
      <div className="game-scrollbar min-h-0 flex-1 space-y-1 overflow-y-auto pr-1">
        {preview.stages.map((stage, index) => {
          const variant =
            index === 0 ? "start" : stage.isFinal ? "final" : "default";
          const requirements = dreamStages?.stages[index]

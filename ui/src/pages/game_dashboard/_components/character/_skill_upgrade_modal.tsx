...
import { SkillUpgradeBenefitLine } from './_skill_upgrade_benefit_line'

interface SkillUpgradeModalProps {
  open: boolean
  preview: SkillUpgradePreview | null
  skillId: string | null
  segmentDisplay: { filled: number; total: number } | null
  balance: number
  onCancel: () => void
  onConfirm: () => void
}
...
export function SkillUpgradeModal({
  ...
  if (!preview || !skillId || !segmentDisplay) return null
  ...
  return (
    ...
    >
      ...
      <div className="mt-4">
        <SkillSegmentBar
          filled={segmentDisplay.filled}
          total={segmentDisplay.total}

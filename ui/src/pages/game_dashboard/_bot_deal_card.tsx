...
import type { bot_deal, bot_deal_side } from './model/types'
import { bot_deal_action_label, format_turns_left_label } from './model/utils'
  ...
  return (
    <span
      className={`inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-lg ${
        playerBuys ? 'bg-emerald-500/15 text-emerald-400' : 'bg-red-500/15 text-red-400'
      }`}
      aria-hidden
    >
  ...
  return (
    ...
    >
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              ...
              <p className="truncate text-xs text-emerald-200/45">{deal.companyName}</p>
            </div>
            <span
          ...
          <div className="mt-3 flex items-center gap-2">
            <span className="font-mono text-xl font-black tracking-wide text-white">{deal.ticker}</span>
            <span className="text-sm font-semibold text-slate-400">×{deal.qty}</span>
      ...
      <div className="mt-4 flex items-center justify-end gap-2">
        <GameButton variant="muted" size="sm" onClick={() => onDecline(deal.id)}>
          Отклонить
        </GameButton>

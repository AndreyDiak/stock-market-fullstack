...
import type { news_item } from './model/types'
import { is_insider_news, news_border_class } from './model/utils'
import { InsiderNewsBadge } from './_insider_news_badge'

export function NewsBlock({ news, theme }: { news: news_item[]; theme: GameDashboardThemeTokens }) {
  return (
    <section
              ...
              <div className="mb-1 flex items-center justify-between gap-2">
                ...
                <span
                  className={`shrink-0 text-[10px] font-bold uppercase tracking-wider ${
                    insider ? 'text-amber-300/90' : item.hot ? 'text-emerald-400' : theme.secondaryText
                  }`}
                >
                  {item.timeLabel}
                </span>

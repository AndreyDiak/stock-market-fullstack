...
import { MOCK_AVAILABLE_CASH, MOCK_NEWS, MOCK_OTC_DEALS, MOCK_PORTFOLIO } from './model/mocks'
...
export function GameDashboardPage() {
  ...
  const { dynamicBackground, colorTheme, setDynamicBackground, setColorTheme } = useGameSettingsStore()
  const dashboardTheme = useMemo(() => getGameDashboardTheme(colorTheme), [colorTheme])
  ...
  const [characterProfile, setCharacterProfile] = useState<CharacterProfile>(MOCK_CHARACTER_PROFILE)
  const [characterUpgrades, setCharacterUpgrades] = useState<CharacterUpgrade[]>(() =>
  ...
  const [bankSummary, setBankSummary] = useState<BankSummary>(MOCK_BANK_SUMMARY)
  const [propertySlots, setPropertySlots] = useState<PropertySlot[]>(() =>
    MOCK_PROPERTY_SLOTS.map((slot) => ({ ...slot, item: slot.item ? { ...slot.item } : undefined })),
  )
  const creditRating = 'A+'

  const propertySlotUpgrade = characterUpgrades.find((item) => item.id === PROPERTY_SLOT_UPGRADE_ID)
    ...
    void fetchGameNews(gameId)
      .then((response) => {
        if (response.news.length > 0) {
          setNews(response.news.map((item, index) => mapApiNewsToFeedItem(item, index)))
        }
      })
      .catch(() => {
  ...
  function handlePurchaseUpgrade(upgradeId: string) {
    ...
    if (upgradeId === PROPERTY_SLOT_UPGRADE_ID && !hasLockedPropertySlots(propertySlots)) return

    setBalance((current) => current - price)
    setCharacterUpgrades((items) =>
      items.map((item) => (item.id === upgradeId ? { ...item, level: item.level + 1 } : item)),
    )
    ...
    try {
      if (gameId) {
        ...
        if (result.news.length > 0) {
          const freshNews = result.news.map((item, index) => mapApiNewsToFeedItem(item, index))

...
import {
  ...
  NEGOTIATE_BASE_MAX_DISCOUNT_PERCENT,
...
describe('negotiate discount by trading level', () => {
  it('maps trading grades F through A to discount caps', () => {
    expect(getMaxNegotiateDiscountPercent(1)).toBe(25);
    expect(getMaxNegotiateDiscountPercent(2)).toBe(30);
    expect(getMaxNegotiateDiscountPercent(3)).toBe(35);
    expect(getMaxNegotiateDiscountPercent(4)).toBe(40);
    expect(getMaxNegotiateDiscountPercent(5)).toBe(45);
    expect(getMaxNegotiateDiscountPercent(6)).toBe(50);
  });

  it('exposes slider and base constants', () => {
    expect(NEGOTIATE_BASE_MAX_DISCOUNT_PERCENT).toBe(20);
  ...
  it('normalizes negotiate percent for trading level', () => {
    expect(normalizeNegotiatePercent(23, 2)).toBe(30);
    expect(normalizeNegotiatePercent(55, 1)).toBe(25);
